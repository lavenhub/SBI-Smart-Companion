...
import { SYSTEM_VAULT_FOLDERS } from '../constants';
import { seedSearchIndex } from './seedSearchIndex';
...
async function seedUsers() {
  const hash = await bcrypt.hash('SBI@12345', 12);
...
async function seedUserData(userId: string, merchantMap: Record<string,string>) {
  const now = new Date();
  ...
  await prisma.card.create({ data: {
    ...
    cvvHash: await bcrypt.hash('456',10),
  ...
  await prisma.card.create({ data: {
    ...
    cvvHash: await bcrypt.hash('789',10),
...
async function seedFinancialProducts(userId: string, savId: string) {
  const now = new Date();
  ...
  for (let i=1; i<=6; i++) {
    const d = new Date('2024-01-05'); d.setMonth(d.getMonth()+i-1);
...
async function seedVaultAndDocs(userId: string) {
  const folderIds: Record<string,string> = {};
  for (let i=0; i<SYSTEM_VAULT_FOLDERS.length; i++) {
    const f = SYSTEM_VAULT_FOLDERS[i];
...
async function seedTransactions(userId: string, savId: string, merchantMap: Record<string,string>) {
  const now = new Date();
  const txns = [
...
async function seedDashboardAndActivity(userId: string) {
