...
exports.seedSearchIndex = seedSearchIndex;
...
async function seedSearchIndex() {
    try {
        const count = await database_1.prisma.searchIndex.count();
        if (count > 0)
            return; // already seeded
        await database_1.prisma.searchIndex.createMany({
        ...
        logger_1.logger.info(`Search index seeded with ${SEARCH_INDEX_DATA.length} features`);
    }
    catch (err) {
        logger_1.logger.warn('Failed to seed search index', { error: err.message });
    }
}
//# sourceMappingURL=seedSearchIndex.js.map